...
class GMM_Clustering:
    def __init__(self, X, K=2, theta=None):
        ...
        self.theta = theta
        self.mu = X[np.random.choice(n, k, False), :]
        self.sigma = [np.eye(d) for i in range(k)]
        self.respon = np.zeros((n, self.k))
    ...
    def M_step(self):
        ...
        for k in range(self.k):
            ## means
            mu_n[k] = 1. / N_ks[k] * np.sum(self.respon[:, k] * X.T, axis = 1).T
